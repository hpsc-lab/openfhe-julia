...
@testset "MyOpenFHE" begin
    ...
    depth = levels_available_after_bootstrap + MyOpenFHE.GetBootstrapDepth(level_budget, secret_key_distribution)
    MyOpenFHE.SetMultiplicativeDepth(parameters, depth)

    cc = MyOpenFHE.GenCryptoContext(parameters)

    MyOpenFHE.Enable(cc, PKE)
    MyOpenFHE.Enable(cc, KEYSWITCH)
    MyOpenFHE.Enable(cc, LEVELEDSHE)
    MyOpenFHE.Enable(cc, ADVANCEDSHE)
    MyOpenFHE.Enable(cc, FHE)
    ...
    MyOpenFHE.EvalBootstrapSetup(cc; level_budget)
    ...
    ptxt = MyOpenFHE.MakeCKKSPackedPlaintext(cc, x; scale_degree = 1, level = depth - 1)
    ...
    ciphertext_after = Compress(cc, ciphertext, 2)

    println("Number of levels remaining after bootstrapping: ", depth - MyOpenFHE.GetLevel(ciphertext_after))
    println()

    result = MyOpenFHE.Plaintext()
    MyOpenFHE.Decrypt(cc, privkey, ciphertext_after, result);
    MyOpenFHE.SetLength(result, encoded_length)
